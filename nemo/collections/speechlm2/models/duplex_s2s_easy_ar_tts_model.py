...
from nemo.collections.speechlm2.modules import (
    EOUDecoder,
    EOUDecoderFromWav,
)
...
from nemo.collections.speechlm2.modules.easy_ar_tts import is_first_process
...
def generate_multiturn_speaking_mask(
    input_ids: torch.Tensor, bos_token_id: int = 0, eos_token_id: int = 1
):
    """
                ...
                if span_len > min_preserve:
                    ...
                    while j < allowed_end:
                        if random.random() < span_prob:
                            ...
                            noised_mask[b, j:flip_end] = (
                                noised_mask[b, j:flip_end] + 1
                            ) % 2
                            j = flip_end
                        else:
                            j += 1
...
class EfficientBatchStreamingSpeakingMaskGenerator:
    def __init__(
        ...
        self.target_mask = torch.zeros(
            batch_size, max_length, dtype=torch.float32, device=device
        )
        self.eou_cache = torch.full(
            (batch_size, max_length), float("nan"), device=device
        )

        self.eou_buffer = torch.zeros(
            batch_size, eou_window, dtype=torch.float32, device=device
        )
        self.eou_ptr = torch.zeros(batch_size, dtype=torch.long, device=device)

        self.recent_is_eos = torch.zeros(
            batch_size, eos_lookback, dtype=torch.bool, device=device
        )
        self.recent_eos_ptr = torch.zeros(batch_size, dtype=torch.long, device=device)

        self.recent_is_bos = torch.zeros(
            batch_size, bos_lookback, dtype=torch.bool, device=device
        )  # NEW
        self.recent_bos_ptr = torch.zeros(
    ...
    def step(
        self, tokens: torch.Tensor, eou_probs: torch.Tensor = None
    ) -> torch.Tensor:
        B = self.batch_size
        device = self.device
        ...
        if eou_probs is not None:
            assert (
                eou_probs.shape[0] == B
            ), f"eou_probs must be shape [B], got {eou_probs.shape}"
        ...
        is_speaking = torch.where(
            is_eos | forced_eos, torch.tensor(1.0, device=device), is_speaking
        )
...
class DuplexS2SEasyARTTSModel(LightningModule, HFHubMixin):
    def __init__(self, cfg: dict) -> None:
        ...
        if self.cfg.get("use_eou_decoder", None):
            if self.cfg.get("eou_decoder_from_wav", None):
                ...
                self.eou_decoder = EOUDecoder(
                    input_dim=self.cfg.get("asr_emb_dim", 512), params=t_params
                )
        ...
        if self.cfg.get("llm_predict_eou", None):
            if self.cfg.get("llm_use_extra_eou_waveform_encoder", False):
                self.eou_wav_encoder = EOUDecoderFromWav(
                    ...
                    n_layers=self.cfg.get(
                        "llm_extra_eou_waveform_encoder_num_layers", 3
                    ),
                    d_model=1024,
                    d_ffn=4096,
                    is_causal=True,
        ...
        if self.cfg.get("pretrained_tts_from_s2s", None):
            self.init_speech_generation_from_another_s2s_checkpoint(
                self.cfg.pretrained_tts_from_s2s
            )
        ...
        self.register_buffer(
            "_control_codes",
            torch.tensor(
                [self.speech_bos_id, self.speech_eos_id, self.speech_delay_id],
                device=self.device,
            ),
        )
    ...
    def init_speech_generation_from_tts_checkpoint(self, checkpoint_path):
        if checkpoint_path is not None:
            ...
            checkpoint_state = set_model_dict_for_partial_init(
                checkpoint_state, self.speech_generation.state_dict()
            )
    ...
    def init_speech_generation_from_another_s2s_checkpoint(self, checkpoint_path):
        if checkpoint_path is not None:
            ...
            checkpoint_state = {
                k.replace("model.speech_decoder.", "").replace(
                    "speech_generation.", ""
                ): v
    ...
    def init_eou_from_another_s2s_checkpoint(self, checkpoint_path):
        if checkpoint_path is not None:
            ...
            # filter keys to keep only speech generation keys and also
            checkpoint_state = {
                k.replace("eou_decoder.", ""): v
                for k, v in checkpoint_state.items()
                if "eou_decoder." in k
            }
            if self.cfg.get("use_eou_decoder", None) or self.cfg.get(
                "inference_use_external_eou_predictor", None
            ):
                checkpoint_state = set_model_dict_for_partial_init(
                    checkpoint_state, self.eou_decoder.state_dict()
                )
    ...
    def forward(
        ...
        if not self.training:
            ...
            if (
                self.cfg.get("inference_force_bos_eos_follow_eou", None)
                and eou is not None
            ):
                ...
                target_text_tokens[:, -1] = torch.where(
                    should_pad, self.text_pad_id, target_text_tokens[:, -1]
                )
        else:
            # drop text bos/eos
            if (
                self.cfg.get("drop_text_bos_prob", None)
                and random.random() < self.cfg.drop_text_bos_prob
            ):
                target_text_tokens = torch.where(
                    target_text_tokens == self.text_bos_id,
                    self.text_pad_id,
                    target_text_tokens,
                )

            if (
                self.cfg.get("drop_text_eos_prob", None)
                and random.random() < self.cfg.drop_text_eos_prob
            ):
                target_text_tokens = torch.where(
                    target_text_tokens == self.text_eos_id,
                    self.text_pad_id,
                    target_text_tokens,
                )
        ...
        with fp32_precision(), torch.autocast(
            ...
            audio_eos_loss, audio_z_loss, audio_k_loss = self.speech_decoder(
                code=target_audio_tokens.clamp_max(self._codebook_size),
    ...
    def forward_infer(
        ...
        subword_mask = torch.ones_like(target_text_tokens).bool()
        cas_emb = self.speech_decoder.cas_encoder(target_text_tokens, subword_mask)
        cond = cond + self.speech_decoder.cas_proj(cas_emb)
        decoder_input_emb = self.speech_decoder.depthsum_embedding(
            input_audio_tokens.clamp_max(self._codebook_size),
            self.speech_decoder.mus,
            include_blank=True,
        ...
        for i in range(batch_size):
            ...
            if random.random() < noise_prob_scale_user:
                scaling_factor = get_scale_factor(
                    ...
                    batch_audio[i],
                    random.randint(
                        noise_prob_scale_user_min_snr, noise_prob_scale_user_max_snr
                    ),
                )
            ...
            def get_noise(noise_files):
                ...
                if noise_resample and sr != self.source_sample_rate:
                    noise = librosa.resample(
                        noise, orig_sr=sr, target_sr=self.source_sample_rate
                    )
                ...
                noise_tensor = torch.tensor(
                    noise, dtype=batch_audio.dtype, device=batch_audio.device
                )
            ...
            if noise.size(0) < audio_length:
                ...
                start_idx = torch.randint(
                    0, noise.size(0) - audio_length + 1, (1,)
                ).item()
    ...
    def prepare_inputs(self, batch: dict):
        ...
        if self.cfg.get("use_old_noise_aug", None):
            ...
            if self.training and random.random() < self.cfg.get(
                "noise_prob_scale_user", 0.0
            ):
        ...
        target_codes = torch.where(
            btt == self.text_bos_id, self.speech_bos_id, target_codes
        )
        target_codes = torch.where(
            btt == self.text_eos_id, self.speech_eos_id, target_codes
        )
        ...
        if self.advance_text_channel_by:
            ...
            target_tokens = torch.cat(
                [target_tokens[:, self.advance_text_channel_by :], pad], dim=-1
            )
        ...
        if self.cfg.get("use_eou_decoder", None):
            ...
            if (
                self.cfg.get("eou_decoder_ignore_sil_batch", False)
                ...
                if self.cfg.get("eou_decoder_from_wav", None):
                    eou_logits, _ = self.eou_decoder(
                        batch["source_audio"].to(source_encoded.dtype),
                        batch["source_audio_lens"],
                    )
                else:
                    eou_logits = self.eou_decoder(
                        asr_emb[:, :-1],
                        seq_mask[:, :, -1].reshape(seq_mask.size(0), seq_mask.size(1)),
                    )
        ...
        if self.cfg.get("llm_predict_eou", None):
            ...
            if self.cfg.get("llm_eou_bos_eos_delay", 0):
                eou_labels = F.pad(
                    eou_labels[:, : -self.cfg.llm_eou_bos_eos_delay],
                    (self.cfg.llm_eou_bos_eos_delay, 0),
                    value=0.0,
                )
                eou_input = F.pad(
                    eou_input[:, : -self.cfg.llm_eou_bos_eos_delay],
                    (self.cfg.llm_eou_bos_eos_delay, 0),
                    value=0.0,
                )
            ...
            eou_loss_scale = seq_mask[:, :, 0].clone().float()
            if (
                self.cfg.get("eou_ignore_sil_batch", False)
                and "silence_augmented" in batch["formatter"][0]
            ):
                eou_skip_batch = True
        ...
        if self.cfg.get("scale_loss_by", None):
            if self.cfg.scale_loss_by == "non_sil_t":
                ...
                for i in range(text_labels.size(0)):
                    ...
                    num_real_padding_tokens = (
                        torch.numel(current_scale) - current_scale.sum()
                    ).item()
                    ...
                    num_non_silence = (
                        torch.numel(silence_idxs) - num_real_padding_tokens
                    )
                    ...
                    loss_scale[i, :, :1] = torch.where(
                        silence_idxs, new_weight, loss_scale[i, :, :1]
                    )
                ...
                loss_scale[:, :, :1] = torch.where(
                    labels == self.text_bos_id, 6.0, loss_scale[:, :, :1]
                )
                loss_scale[:, :, :1] = torch.where(
                    labels == self.text_eos_id, 6.0, loss_scale[:, :, :1]
                )
                ...
                loss_scale[:, :, :1] = torch.where(
                    text_labels_exp
                    != self.text_pad_id,  # Condition: not a padding token
                ...
                # Shape: [B]
                num_bos_tokens = (
                    (text_labels_exp == self.text_bos_id).flatten(1, 2).sum(-1)
                )

                # Count how many EOS tokens are present per sequence
                # Shape: [B]
                num_eos_tokens = (
                    (text_labels_exp == self.text_eos_id).flatten(1, 2).sum(-1)
                )
                ...
                for i in range(text_labels.size(0)):
                    ...
                    if tot_special_tokens[i] > 0:
                        ...
                        new_weight = (
                            tot_scale_for_valid_tokens[i] / tot_special_tokens[i]
                        )
                    else:
                ...
                self.cfg.scale_loss_by == "non_sil_4_dynamic_x_less_eos_bos_speech_text"
            ):
                # Expand text_labels to match the shape of loss_scale: [B, T] → [B, T, 1]
                text_labels_exp = text_labels.unsqueeze(-1)
                ...
                # Compute the total loss weight assigned to valid (non-padding) tokens in each sequence
                # Shape: [B] — one scalar value per batch item
                text_tot_scale_for_valid_tokens = (
                ...
                for i in range(text_labels.size(0)):
                    ...
                    if tot_special_tokens[i] > 0:
                        # Redistribute the total valid token weight equally across BOS and EOS tokens
                        new_weight_text = (
                            text_tot_scale_for_valid_tokens[i] / tot_special_tokens[i]
                    ...
                    # Assign new_weight to BOS tokens in the current sequence
                    loss_scale[i, :, 1:] = torch.where(
                        audio_labels[i, :, :] == self.speech_bos_id,
                        new_weight_speech,
                        loss_scale[i, :, 1:],
                    )
                    ...
                    # Assign new_weight to EOS tokens in the current sequence
                    loss_scale[i, :, 1:] = torch.where(
                        audio_labels[i, :, :] == self.speech_eos_id,
                        new_weight_speech,
                        loss_scale[i, :, 1:],
                    )

            elif (
                self.cfg.scale_loss_by
                == "non_sil_4_dynamic_10x_less_eos_bos_speech_text"
            ):
                # Expand text_labels to match the shape of loss_scale: [B, T] → [B, T, 1]
                text_labels_exp = text_labels.unsqueeze(-1)
        ...
        if (
            ...
            def count_leading_silence_tokens(
                tensor: torch.Tensor, silence_token: int = 0
            ) -> int:
                """
            ...
            with fp32_precision(), torch.no_grad():
                lengths = torch.tensor(
                    [batch["target_audio"].shape[1]] * batch["target_audio"].shape[0]
                ).to(self.audio_codec.device)
                reconstructed_audio_from_wav, _ = self.audio_codec(
                    audio=batch["target_audio"], audio_len=lengths
                )
                # reconstruct wav
                audio_labels_ = replace_control_speech_codes(
                    audio_labels, self._control_codes
                )

                lengths = torch.tensor(
                    [audio_labels_.shape[1]] * audio_labels_.shape[0]
                ).to(self.audio_codec.device)
            ...
            for i in range(audio_labels_.shape[0]):
                ...
                if self.cfg.get("use_eou_decoder", None) or self.cfg.get(
                    ...
                    eou_wav = (
                        ...
                        .unsqueeze(0)
                        .unsqueeze(-1)
                        .repeat(1, 1, repeat_factor)
                    )  # (B, T, repeat_factor)
                    eou_wav = eou_wav.view(1, -1)  # (B, T * repeat_factor)
                    eou_wav = (
                        eou_wav.float() * 0.8
                    )  #  make 1 audible and keep 0 as total silence
                    write_wave(
                        eou_wav.squeeze(),
                        os.path.join(
                            self.cfg.get("debug_dataloader_audios_path"), f"eou_{i}.wav"
                        ),
                        sr=self.target_sample_rate,
                    )

            num_bos_tokens = (
                (text_labels.unsqueeze(-1) == self.text_bos_id).flatten(1, 2).sum(-1)
            )
            # Count how many EOS tokens are present per sequence
            # Shape: [B]
            num_eos_tokens = (
                (text_labels.unsqueeze(-1) == self.text_eos_id).flatten(1, 2).sum(-1)
            )
            ...
            print(
                ...
                count_leading_silence_tokens(
                    text_labels[-1:].squeeze(), self.text_pad_id
                ),
            )

            print(batch["formatter"])
            if (
                audio_labels_.shape[0] > 1
                and num_bos_tokens[0].item() == num_eos_tokens[0].item()
            ):
                exit()
    ...
    def track_param_updates(
        self, param_filter: str = "speech_generation.text_embeddings", verbose=True
    ):
        if not hasattr(self, "_param_tracker_state"):
            ...
            if verbose:
                print(
                    f"[Tracker] Initialized snapshot for: {[k for k in self._param_tracker_state]}"
                )
            return
        ...
        for name, p in self.named_parameters():
            ...
            if delta.item() == 0.0:
                ...
                print(
                    f"✅ {name:50s} Δsum={delta.item():.4e}  Δmean={mean_delta.item():.4e}"
                )
    ...
    def training_step(self, batch: dict, batch_idx: int):
        ...
        with loss_parallel():
            ...
            # mask audio logits to ignore sequence padding
            audio_loss = forward_outputs[
                "audio_loss"
            ]  # TODO(jaehyeonk): enable loss_mask and seq_mask
        loss = (
        ...
        if self.cfg.get("llm_predict_eou", None) and not inputs["eou_skip_batch"]:
            eou_loss = (
                torch.nn.functional.cross_entropy(
                    forward_outputs["eou_logits"].flatten(
                        0, 1
                    ),  # (B, T, Vt) -> (*, Vt)
                    inputs["eou_labels"].flatten(0, 1),
                    reduction="none",
        ...
        for name, dataset_batch in batch.items():
            ...
            with fp32_precision():  # resample is fragile to bfloat16 default dtype
                ...
                self.results_logger.update(
                    ...
                    pred_audio_sr=self.audio_codec_sample_rate,
                    ...
                    fps=self.source_fps,
                    results=results
                    if self.cfg.get("dump_tokens_text", False)
                    else None,
                    tokenizer=self.tokenizer,
                )
            ...
            self.text_bos_acc.update(
                name=name,
                refs=dataset_batch["target_tokens"],
                hyps=results["tokens_text"],
            )
            self.text_eos_acc.update(
                name=name,
                refs=dataset_batch["target_tokens"],
                hyps=results["tokens_text"],
            )
        ...
        with fp32_precision(), torch.autocast(
            ...
            if self.cfg.get("inference_eou_from_bos_eos", None):
                eou_mask_generator = EfficientBatchStreamingSpeakingMaskGenerator(
                    ...
                    eos_lookback=7,
                    bos_lookback=7,
                    force_bos_from_eou=self.cfg.get(
                        "inference_force_follow_external_eou_bos", False
                    ),
                )
            ...
            if self.cfg.get("use_eou_decoder", None) or self.cfg.get(
                ...
                if self.cfg.get("eou_decoder_from_wav", None) or self.cfg.get(
                    ...
                    eou_logits, _ = self.eou_decoder(
                        input_signal.to(source_encoded.dtype), input_signal_lens
                    )
            ...
            gen_audio = torch.empty(
                B, T, self._num_codebooks, device=self.device, dtype=torch.long
            )
            ...
            if self.cfg.get("inference_eou_from_bos_eos", None):
                gen_eou[:, 0] = eou_mask_generator.step(
                    gen_text[:, 0],
                    eou_probs=external_eou[:, 0]
                    if self.cfg.get("inference_force_follow_external_eou", None)
                    else None,
                )[:, 0]
            ...
            for t in range(1, T):
                ...
                num_transition_tokens = self.cfg.get(
                    "inference_eou_num_transition_tokens", 2
                )
                if (
                    self.cfg.get("inference_force_bos_eos_follow_eou", None)
                    and t >= num_transition_tokens
                ):
                    # Check last num_transition_tokens EOU predictions: if all 0 and current is 1 → BOS in text
                    prev_zeros = (gen_eou[:, t - num_transition_tokens : t] == 0).all(
                        dim=1
                    )
                    ...
                    gen_text[:, t] = torch.where(
                        force_bos, self.text_bos_id, gen_text[:, t]
                    )

                    # Check last num_transition_tokens EOU predictions: if all 1 and current is 0 → EOS in text
                    prev_ones = (gen_eou[:, t - num_transition_tokens : t] == 1).all(
                        dim=1
                    )
                    ...
                    # force eos
                    gen_text[:, t] = torch.where(
                        force_eos, self.text_eos_id, gen_text[:, t]
                    )

                    not_special = (gen_text[:, t] != self.text_bos_id) & (
                        gen_text[:, t] != self.text_eos_id
                    )
                    should_pad = (gen_eou[:, t] == 0) & not_special

                    # if EOU is zero, allows text channel only to assume, zero, eou or bos
                    gen_text[:, t] = torch.where(
                        should_pad, self.text_pad_id, gen_text[:, t]
                    )

                if (
                    self.cfg.get(
                        "inference_force_bos_eos_follow_eou_speech_channel", None
                    )
                    and t >= num_transition_tokens
                ):
                    not_special = (gen_audio[:, t] != self.speech_bos_id) & (
                        gen_audio[:, t] != self.speech_eos_id
                    )
                ...
                if (
                    ...
                    gen_audio[:, -1] = torch.where(
                        (
                            gen_text[:, -1 - num_speech_delay].unsqueeze(-1)
                            == self.text_bos_id
                        )
                        * (
                            torch.sum(
                                gen_audio[:, -1 - num_speech_delay :]
                                == self.speech_bos_id,
                                1,
                            )
                            == 0
                    ...
                    gen_audio[:, -1] = torch.where(
                        (
                            gen_text[
                                ...
                                -1
                                - num_speech_delay
                                - self.cfg.get("advance_text_channel_by", 0),
                            ].unsqueeze(-1)
                            == self.text_eos_id
                        ),
                        self.speech_eos_id,
            ...
            if decode_audio:
                gen_audio_codes = replace_control_speech_codes(
                    gen_audio, self._control_codes
                )
                predicted_audio_list = []
                predicted_audio_lens_list = []
                for i in range(gen_audio_codes.size(0)):
                    predicted_audio, predicted_audio_lens = self.audio_codec.decode(
                        gen_audio_codes[i : i + 1, : lengths[i]].clamp_max(
                            self._codebook_size
                        ),
                        lengths[i : i + 1],
                    ...
                    predicted_audio_lens_list.append(predicted_audio_lens)
                predicted_audio_lens = torch.cat(predicted_audio_lens_list, 0)
                max_predicted_audio_len = predicted_audio_lens.max().item()
                predicted_audio = torch.cat(
                    [
                        F.pad(x, [0, max_predicted_audio_len - x.size(-1)])
                        for x in predicted_audio_list
                    ],
                    0,
                )
    ...
    def oomptimizer_schema(self) -> dict:
        ...
        return {
            ...
            "inputs": [
                {
                    "name": "source_audio",
                    "type": NeuralType(("B", "T"), AudioSignal()),
                    "seq_length": "input",
                },
                {
                    "name": "source_audio_lens",
    ...
    def configure_model(self) -> None:
        ...
        if (dp_mesh := device_mesh["data_parallel"]).size() > 1:
            ...
            self.speech_decoder = fully_shard(self.speech_decoder, **fsdp_config)
